<template>
    <div class="container">
        <h1 class="search-header">Search Results</h1>
        
        <p class="description">The search term <span style="color: darkblue">"{{ searchQuery }}"</span> 
        appears in the following pages: <span><strong style="color: darkblue">{{ totalCount }} times</strong></span></p>

        <div class="spinner" v-if="isLoading"><Spinner /></div>
        <SearchCard v-else v-for="item in searchResults" :key="item.Title" :resultPage="item" />
        
        <p style="margin-bottom: 60px;"></p>
    </div>
</template>


<script setup>
import { ref, onMounted, watch } from 'vue';
import { onBeforeRouteLeave, useRoute } from 'vue-router';
import SearchData from "../data/SearchData.json";
import SearchCard from "../components/SearchCard.vue";
import Spinner from "../components/LoadingSpinner.vue";

const route = useRoute();
const searchQuery = ref(route.params.SearchQuery || '');
const searchResults = ref([]);
const totalCount = ref(0);
const isLoading = ref(false);

function highlightString(text, searchQuery) {
    const searchLength = searchQuery.length;
    let highlightedText = '';
    let idx = 0;
    let count = 0;
    const lowerText = text.toLowerCase();
    const lowerSearchQuery = searchQuery.toLowerCase();
    
    while (idx < text.length) {
        const nextFind = lowerText.indexOf(lowerSearchQuery, idx);

        if (nextFind === -1) {
            highlightedText += text.substring(idx);
            break;
        }
        
        highlightedText += text.substring(idx, nextFind) +
                         `<span style="background-color: yellow">${text.substring(nextFind, nextFind + searchLength)}</span>`;
                         
        idx = nextFind + searchLength;
        count++;
    }

    return { highlightedText, count };
}

function searchJson(data, searchQuery) {
    const allMatches = [];
    let totalHighlights = 0;
    
    data.forEach(item => {
        let matchedResults = [];
        let localCount = 0;
        
        if (item.Results) {
            item.Results.forEach(result => {
                let resultMatched = false; 
                
                const newResult = {};
                
                for (const key in result) {
                    const value = result[key];
                    
                    if (typeof value === 'string' && value.toLowerCase().includes(searchQuery.toLowerCase())) {
                        const { highlightedText, count } = highlightString(value, searchQuery);
                        newResult[key] = highlightedText;
                        localCount += count;
                        resultMatched = true;
                    }
                }
                
                if (resultMatched) {
                    matchedResults.push(newResult);
                }
            });
            
            if (matchedResults.length > 0) {
                allMatches.push({
                    Title: item.Title,
                    Link: item.Link,
                    MatchedResults: matchedResults
                });
                
                totalHighlights += localCount;
            }
        }
    });
    
    totalCount.value = totalHighlights;
    return allMatches;
}

function performSearch() {
    isLoading.value = true;
    const delay = Math.floor(Math.random() * (1000 - 500 + 1)) + 500; // Random number between 1000 and 1500

    setTimeout(() => {
        searchResults.value = searchJson(SearchData, searchQuery.value);
        isLoading.value = false;
    }, delay);
}

onBeforeRouteLeave((to, from, next) => {
    window.scrollTo(0, 0);
    next();
});

onMounted(() => {
    performSearch();
});

watch(() => route.params.SearchQuery, (newSearchQuery) => {
    searchQuery.value = newSearchQuery;
    totalCount.value = 0;
    performSearch();
});

</script>


<style scoped>

.container {
    display: flex;
    flex-direction: column;
    align-items: center;
    width: 100%;
}

.spinner {
    margin-top: 60px;
    margin-right: 20px;
}

.search-header {
    text-align: left;
    border-bottom: 1.5px solid #d8dee4;
    padding-bottom: 7px;
    width: 690px;
}

.description {
    margin-top: -8px;
    font-size: 19px;
    width: 690px;
    margin-bottom: 4px;
}

@media (max-width: 700px) {
    .search-header, .description {
        width: 490px;
    }
}

</style>